...
public class StudyBeanImpl implements StudyBean {
	...
	public String getStudyId() {
		log.info("StudyBeanImpl.getStudyId = " + studyId);
		return studyId;
	}

	@Begin(flushMode = FlushModeType.MANUAL, join = true)
	public void setStudyId(String studyId) {
		log.info("StudyBeanImpl.setStudyId = " + studyId);
		this.studyId = studyId;
		try {

			this.study = studyEJB3DAO.getByAccForUser(studyId, identity.getUsername());
			log.info("Study users: " + study.getUsers());
	...
	public String getOrganism() {
		if (organism == null) {
			...
			organism = buildValuesString(values);
		}
	...
	private String buildValuesString(Collection<PropertyValue> values) {
