...
public class ReferenceSourceEJB3DAO extends IdentifiableEJB3DAO<ReferenceSource>
	...
	public ReferenceSourceEJB3DAO(EntityManager entityManager) {
		super(ReferenceSource.class, entityManager);
	}

	public ReferenceSource getReferenceSourceByName(String name) {
		List results = entityManager.createQuery(
				"select r from ReferenceSource r where name = :name").
...
//					"correspond to a name : " + name);
		...
		return (ReferenceSource) results.get(0);
	}

	public ReferenceSource getReferenceSourceByAcc(String acc) {
		List results = entityManager.createQuery(
				"select r from ReferenceSource r where acc = :acc").
